...
title('Polar NRZ channel output fc=500 Hz - Rb=1kbps')
...
subplot(425)
plot(to, NRZ_channel_out, 'b'); hold on;
title('Polar NRZ channel output fc=1000 Hz - Rb=1kbps')
%stem(tx, x, 'kx'); hold off;
axis([0 25 -6 6]);  xlabel('Time (ms)'); ylabel('Amplitude');
fpass=2000; %change!
NRZ_channel_out = lowpass(NRZ_out,fpass,Fs);
subplot(427)
plot(to, NRZ_channel_out, 'b'); hold on;
title('Polar NRZ channel output fc=2000 Hz - Rb=1kbps')
%stem(tx, x, 'kx'); hold off;
axis([0 25 -6 6]);  xlabel('Time (ms)'); ylabel('Amplitude');
%% tek kutup icin
Nsym = 6;           % Filter span in symbol durations
...
subplot(422)
plot(to, NRZ_out, 'r-'); hold off;
title('Unipolar NRZ transmit data')
% Set axes and labels.
axis([0 25 -6 6]);  xlabel('Time (ms)'); ylabel('Amplitude');
legend('Transmitted Data', 'NRZ Coded Data', 'Unipolar Line Coded Data');
fpass=500; %change!
...
subplot(424)
plot(to, NRZ_channel_out, 'b'); hold on;
title(' Unipolar NRZ channel output fc=500 Hz - Rb=1kbps')
%stem(tx, x, 'kx'); hold off;
axis([0 25 -6 6]);  xlabel('Time (ms)'); ylabel('Amplitude');
fpass=1000; %change!
NRZ_channel_out = lowpass(NRZ_out,fpass,Fs);
subplot(426)
plot(to, NRZ_channel_out, 'b'); hold on;
title('Unipolar NRZ channel output fc=1000 Hz - Rb=1kbps')
% stem(tx, x, 'kx'); hold off;
axis([0 25 -6 6]);  xlabel('Time (ms)'); ylabel('Amplitude');
fpass=2000; %change!
NRZ_channel_out = lowpass(NRZ_out,fpass,Fs);
subplot(428)
plot(to, NRZ_channel_out, 'b'); hold on;
title('Unipolar NRZ channel output fc=2000 Hz - Rb=1kbps')
% stem(tx, x, 'kx'); hold off;
axis([0 25 -6 6]);  xlabel('Time (ms)'); ylabel('Amplitude');
